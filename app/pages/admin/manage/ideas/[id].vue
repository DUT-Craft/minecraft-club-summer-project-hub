        ...
        <n-card class="detail-hero" :bordered="false">
          <div class="hero-info">
            <n-space :size="8" align="center">
              <n-tag :bordered="false" :type="statusTagType(idea.reviewStatus)">{{ ideaStatusLabel(idea.reviewStatus) }}</n-tag>
              <n-tag :bordered="false" round class="id-tag">#{{ idea.id }}</n-tag>
            </n-space>
            <h1>{{ idea.title }}</h1>
            <span>{{ idea.nickname || idea.submitterName || "匿名" }}{{ idea.minecraftId ? ` · MC：${idea.minecraftId}` : "" }}</span>
          </div>
        ...
        >
          ...
          >
            ...
            <n-form-item label="正文" path="content">
              <n-input v-model:value="form.content" type="textarea" :rows="6" placeholder="想法正文" />
            </n-form-item>
          </n-form>
